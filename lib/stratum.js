 ...
 * @constructor
 */
var StratumClient = function(options, logger){
    ...
    function handleMessage(message){
        switch(message.method){
            case 'mining.subscribe':
                logger.silly('mining.subscribe message from  miners');
                handleSubscribe(message);
                break;
            case 'mining.authorize':
                logger.silly('mining.authorize message from  miner');
                handleAuthorize(message, true /*reply to socket*/);
                break;
            case 'mining.get_multiplier':
                logger.silly('mining.get_multiplier message from  miner');
                ...
                break;
            case 'ping':
                logger.silly('ping message from  miner');
            ...
            case 'mining.submit':
                _this.lastActivity = Date.now();
                logger.silly('mining.submit message from  miner');
            ...
            case 'mining.get_transactions':
                ...
                logger.silly('mining.get_transactionsfrom  miner, message = %s');
                break;
            default:
    ...
    function handleAuthorize(message, replyToSocket){
        ...
        options.authorizeFn(_this.remoteAddress, options.socket.localPort, _this.workerName, _this.workerPass, function(result) {
            ...
            var isValidAddy = null;
            
            // check if client worker name is valid
            _this.daemon.cmd('validateaddress', _this.workerName, function(result) {
            	
									//Check if payments failed because wallet doesn't have enough coins to pay for tx fees
									if (result.error) {
										
										logger.error('Error trying to validate address with RPC validateaddress %s', JSON.stringify(result.error));
										callback(true);
										
									} else {
										
										if (result.error) {
											logger.warn('We didn\'t get a response from \'validateaddress\'... This could be a problem! Tried parsing: %s', JSON.stringify(result));
										}
										else if (result.response) {
											isValidAddy = result.response.isvalid;
										}

										if (isValidAddy) {
											logger.warn('Address %s is valid!', _this.workerName);
										}
										else {
											logger.warn('Address %s is NOT valid!', _this.workerName);
										}
			...
			_this.authorized = (!result.error && result.authorized && isValidAddy);
 ...
 **/
var StratumServer = exports.Server = function StratumServer(options, authorizeFn, logger){

...
var generateOutputTransactions = function (poolRecipient, recipients, rpcData, poolOptions) {
    ...
    /* Einsteinium */
    if (rpcData.charityvalue && rpcData.charityScript) {
        reward -= rpcData.charityvalue;
        rewardToPool -= rpcData.charityvalue;
    }
    ...
    //infinex, todo change to check by config
    if(rpcData.founderreward) {
        let founderRewardSatoshis = rpcData.founderreward.amount;
        let founderRewardAddress = rpcData.founderreward.founderpayee;

        reward -= founderRewardSatoshis;
        rewardToPool -= founderRewardSatoshis;

        let payeeScript = util.addressToScript(founderRewardAddress);
        txOutputBuffers.push(Buffer.concat([
            util.packInt64LE(founderRewardSatoshis),
            util.varIntBuffer(payeeScript.length),
            payeeScript
        ]));
    }
    ...
    if (rpcData.znode) {
        ...
        let testnet = poolOptions.testnet;

        //Zcoin founders mainnet and testnet
        let zcoinFounders = {
            mainnet: {
        ...
        if (testnet) {
            founders = zcoinFounders.testnet;
        } else {
            founders = zcoinFounders.mainnet;
        }
    ...
    /* Einsteinium */
    if (rpcData.charityvalue && rpcData.charityScript) {
        var charityScript = util.miningKeyToScript(rpcData.charityScript);
        txOutputBuffers.unshift(Buffer.concat([
            util.packInt64LE(rpcData.charityvalue),
            util.varIntBuffer(charityScript.length),
            charityScript
        ]));
    }
